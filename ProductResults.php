...
class ProductResults extends Advanced_Product_Filtering {
		...
		public function __construct($parameter, $id, $searchStr="", $table="CubeCart_inventory") {
		
		$this->product_results($parameter, $id, $searchStr, $table);
		...
		public function product_results($parameter="", $id="", $searchStr="", $table = "CubeCart_inventory") {
		...
		$query = "SELECT productId FROM $table WHERE $parameter IN (".$this->create_csv_id_list($id).")";
		
		}
		
		else {
		
		$query = "SELECT productId FROM $table WHERE $parameter = $id";
