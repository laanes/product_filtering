...
require_once('modules'.CC_DS.'3rdparty'.CC_DS.'Advanced_Product_Filtering'.CC_DS.'APF_DB_BRAND.php');
...
$apf = new APF_DB_BRAND($_GET, "I", $_REQUEST['brandId']);
...
if ($prods_per_cat[0]['product_count'] > 0) {

$link = $apfu->create_link("cat_id", $cat[$i]['cat_id']);
...
$filter_class = $apfu->generate_filter_class("cat_id", $cat[$i]['cat_id']);
...
$link = $apfu->create_link("brand_id", $brand[$i]['brand_id']);
...
$box_content->parse('filter_box.price_filters_heading');

for($i=0; $i<=count($price_ranges)-1; $i++) {
$prods_per_price = $apf->count_prods("price_range", $price_ranges[$i]['range_id'], true);
