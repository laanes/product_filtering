...
class APF_DB_BRAND extends APF_DB {
		...
		public function get_categories($total_params=3, $parameter="") {
		...
		global $db;
		$query = "SELECT B.brand_name, B.brand_id FROM CubeCart_productbrands as B INNER JOIN CubeCart_inventory as I ON B.brand_id = I.brand_id WHERE I.brand_id IN ('". $this->brand_ids ."') GROUP BY B.brand_name";
		$brands = $db->select($query);
		return $brands;
		}
		...
		public function count_prods($parameter="", $id="", $extra_key=""){
		...
		$condition = "AND I.productId IN ('".$this->product_ids."') $url_filter";
		...
		$query = "SELECT COUNT(productId) as product_count FROM CubeCart_inventory as I WHERE $parameter = '$id' $condition";
		$prod_count = $db->select($query);
		return $prod_count;
		
		}
		
		public function get_product_results($brand_id="") {
		
		global $db;
		
		$query = "SELECT * FROM CubeCart_inventory WHERE brand_id = ".$brand_id." AND brand_id > 0";
		
		$productResults = $db->select($query);
		
		return $productResults;
		
		}				public function is_only_parameter($haystack="", $needle="") {				if(strpos($haystack, $needle)) {				return true;				}				else {				return false;				}				}
}
